...
class EmpiricalGravitationalWaveTransient(GravitationalWaveTransient):
    ...
    def __init__(
        ...
        generate_gaussian_background=False,
        empirical_minimum_frequency=20.0,
        empirical_maximum_frequency=100.0,
        data_for_background=None,
    ):
        ...
        if data_for_background is not None:

            self.loglikelihood_object = self.background_distribution_from_timeshifts(
                data_for_background, interferometers
            )
        else:
            if generate_gaussian_background:

                data_for_background = self._gaussian_background_distribution(
                    interferometers
                )

                self.loglikelihood_object = (
                    self.background_distribution_from_timeshifts(
                        gaussian_background_data
                    )
                )
            else:
                self.loglikelihood_object = self._real_data_distributon(interferometers)
    ...
    def background_distribution_from_timeshifts(self, strain_series, interferometers):
        ...
        analysis_duration = int(interferometers[0].duration)
        sampling_frequency = int(interferometers[0].sampling_frequency)
        background_duration = strain_series[0].size / sampling_frequency

        minimum_frequency = self.empirical_minimum_frequency
        maximum_frequency = self.empirical_maximum_frequency
        start_time = interferometers[0].start_time - 0.5 * background_duration

        for ii in range(len(strain_series)):
            strain_series[ii] = TimeSeries(
                strain_series[ii], t0=start_time, dt=1.0 / sampling_frequency
            )
        ...
        for i in range(n_segments):
            ...
            interferometers_segment[0].set_strain_data_from_gwpy_timeseries(
                strain_series[0][idxmin:idxmax]
            )
            interferometers_segment[1].set_strain_data_from_gwpy_timeseries(
                strain_series[1][idxmin:idxmax]
            )
            ...
            try:
                H1_spectrogram = np.vstack(
                    (
                        (
                            H1_spectrogram,
                            interferometers_segment[0].frequency_domain_strain,
                        )
                    )
                )
                L1_spectrogram = np.vstack(
                    (
                        (
                            L1_spectrogram,
                            interferometers_segment[1].frequency_domain_strain,
                        )
                    )
                )
        ...
        H1_spectrogram = Spectrogram(
            H1_spectrogram,
            frequencies=frequency_array,
            t0=start_time,
            dt=analysis_duration,
        )

        L1_spectrogram = Spectrogram(
            L1_spectrogram,
            frequencies=frequency_array,
            t0=start_time,
            dt=analysis_duration,
        ...
        H1_spectrogram = H1_spectrogram.crop_frequencies(
            low=minimum_frequency, high=maximum_frequency
        )
        L1_spectrogram = L1_spectrogram.crop_frequencies(
            low=minimum_frequency, high=maximum_frequency
        )
        ...
        H1_psd = interferometers[
            0
        ].power_spectral_density.power_spectral_density_interpolated(frequency_array)
        L1_psd = interferometers[
            1
        ].power_spectral_density.power_spectral_density_interpolated(frequency_array)

        log_probability_object = self._calcuate_2det_background_from_spectrogram(
            H1_spectrogram.value,
            L1_spectrogram.value,
            H1_psd,
            L1_psd,
            analysis_duration,
        )
    ...
    def _calcuate_2det_background_from_spectrogram(
        self, H1, L1, H1_psd, L1_psd, analysis_duration
    ):
        ...
        kde = KernelDensity(kernel="gaussian", bandwidth="scott").fit(
            gamma.reshape(gamma.size, 1)
        )

        return kde

    def _masked_inner_products(
        self, frequency_domain_strain, signal, power_spectral_density, mask
    ):
        ...
        inner_products["h_inner_h"] = optimal_snr_squared(
            signal=signal[mask],
            power_spectral_density=power_spectral_density[mask],
    ...
    def noise_log_likelihood(self):
        ...
        if self._noise_log_likelihood_value is None:
            ...
            gamma_log_likelihood = self.loglikelihood_object.score_samples(
                np.array(noise_gamma).reshape(1, -1)
            )[0] - (self.n_dof - 1) * np.log(noise_gamma)

            self._noise_log_likelihood_value = (
                gamma_log_likelihood + gaussian_noise_logl
            )
    ...
    def _empirical_log_likelihood(self, waveform_polarizations):
        ...
        for interferometer in self.interferometers:
            ...
            inner_products = self._masked_inner_products(
                interferometer.frequency_domain_strain,
                signal,
                interferometer.power_spectral_density_array,
                interferometer.gamma_frequency_mask,
        ...
        loglike_gamma = self.loglikelihood_object.score_samples(
            np.array(gamma).reshape(1, -1)
        )[0]
    ...
    def log_likelihood(self):
        ...
        empirical_log_likelihood = self._empirical_log_likelihood(
            waveform_polarizations
        )
        noise_log_likelihood = self.noise_log_likelihood()

        return empirical_log_likelihood + noise_log_likelihood
