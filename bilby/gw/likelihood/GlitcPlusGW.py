...
class GlitchIncludedGravitationalWaveTransient(GravitationalWaveTransient):
    ...
    @classmethod
    def frequency_domain_wavelet(A, t0, sigma, frequencies):

        wavelet_term1 = A * 1j / (2 * np.pi * frequencies) * np.exp(-2 * np.pi * 1j * frequencies * t0 ) 
        wavelet_term2 = np.exp( - (np.pi * frequencies * sigma)**2)

        return wavelet_term1 * wavelet_term2
    ...
    def _glitch_inclusive_likelihood(self, waveform_polarizations):
        ...
        for interferometer in self.interferometers:
            ...
            ifo_data = interferometer.frequency_domain_strain

            if interferometer.name in self.ifos_with_glitch:
